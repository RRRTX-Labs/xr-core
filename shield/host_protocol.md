...
| `bundle-check` | `{bundle, manifest}` | parse + bind against a frozen `list-bundle-manifest-v1` document: per entry, `name` equal, `rules` count equal, `sha256` == digest of that list's canonical bytes. Bound ⇒ `{"bound":true,"bundle_version","digest","list_count"}`; any mismatch ⇒ `kRejected` with the binding token; malformed manifest ⇒ `kMalformedInput` |
...
- `kRejected` (exit 0) — a typed REFUSAL, not a protocol error. Reason
  ...
  - manifest binding: `manifest-list-count`, `manifest-list-name:<name>`,
    `manifest-rule-count:<name>`, `manifest-sha256:<name>`.
